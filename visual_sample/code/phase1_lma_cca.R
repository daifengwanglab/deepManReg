...
#lma
NMA_res = ManiNetCluster(X,Y,nameX='Ephys',nameY='Expr',corr=corr,method = method[1],d=3L,k_NN=5L,k_medoids=5L)
x = NMA_res[3:5]
write.csv(x,'../data/ma_latent.csv',row.names = F)

#cca
NMA_res = ManiNetCluster(X,Y,nameX='Ephys',nameY='Expr',corr=corr,method = method[2],d=3L,k_NN=5L,k_medoids=5L)
x = NMA_res[3:5]
